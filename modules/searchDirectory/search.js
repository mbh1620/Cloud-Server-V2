var editFileTypes = ['txt', 'html', 'js', 'py', 'json']
...
function updateList(returnedFiles, isEditFlag){
    ...
    for(var i = 0; i < returnedFiles.length; i++){
        ...
        var listHTML = ""
        var content = "<button onclick='deleteFile(`" + returnedFiles[i].filename + "`)' style='float:right;margin-left:20px; margin-top:0px;' class='btn btn-sm btn-danger'>Delete</button><button onclick='renameitem(`" + returnedFiles[i].filename + "`)' style='float:right;margin-left:20px; margin-top:0px;' class='btn btn-sm btn-warning'>Rename</button><button onclick='moveItem(`" + returnedFiles[i].filename + "`)' style='float:right;margin-left:20px; margin-top:0px;' class='btn btn-sm btn-primary'>Move</button>"

        if(editFileTypes.includes(returnedFiles[i].fileExtension) && isEditFlag == true) {
            listHTML = content + "<button onclick='editTextFile(`" + returnedFiles[i].filename + "`)' style='float:right;margin-left:20px; margin-top:0px;' class='btn btn-sm btn-primary'>Edit</button>"
        } else if( isEditFlag == true ){
            listHTML = content
        } else {
            listHTML = ""
        }
